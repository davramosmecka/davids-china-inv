...
function renderStock() {
  ...
  grid.innerHTML = filtered.map(item => {
    ...
    const thumbHtml = item.photo
      ? `<img class="inv-card-thumb" src="${item.photo}" alt="${item.name}" onclick="openLightbox('${item.id}')">`
      : '';
    return `
...
function saveEditItem() {
  ...
  // Save photo — use current preview src (may be new upload or existing)
  const preview = document.getElementById('edit-item-preview');
  item.photo = preview.style.display !== 'none' && preview.src ? preview.src : null;
  saveState();
